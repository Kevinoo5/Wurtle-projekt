...
def letter5():
    ...
    def get_word():
        ...
        while len(count) < 7:
            proov = word.get()
            if len(proov) != 5:
                tk.messagebox.showwarning("showwarning", "OH NO!!! This is not in the list")
                word.delete(0, 10)
                return None
                break
            ...
            sona = c.fetchall()
            if len(count) >= 5:
                tk.messagebox.showinfo("WORD", "The word was: "+suva[0][1])
                c.close()
                ühendus.close()
                exit()
            if sona != []:
            ...
            if proov in sona:
                for i in range(integer):
                    if proov == suva[0][1]:
                        tk.messagebox.showinfo("U won!", "U so smartest! Be proud of yourself!")
                        true = True
                        exit()
